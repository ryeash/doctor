...
public class StructuredConfigurationSource implements ConfigurationSource {
    ...
    public static class StructuredPropertiesParser {
        ...
        public Stream<Token> streamTokens() {
            AtomicReference<Token> previous = new AtomicReference<>(new Token(StructuredPropertiesParser.Type.INITIAL, null, null, 0));
            ...
            return Stream.generate(() -> {
                    ...
                    .filter(t -> !(t.ttype == TT_EOL || t.ttype == ConfigurationFacade.LIST_DELIMITER))
                    .map(tokenizer -> {
                        int ttype = tokenizer.ttype;
                        String sval = tokenizer.sval;
                        int line = tokenizer.lineno();
                        Token token = switch (ttype) {
                            case '{' -> new Token(StructuredPropertiesParser.Type.OPEN_OBJECT, previous.get().field, null, line);
                            case '}' -> new Token(StructuredPropertiesParser.Type.CLOSE_OBJECT, null, null, line);
                            case '[' -> new Token(StructuredPropertiesParser.Type.OPEN_ARRAY, previous.get().field, null, line);
                            case ']' -> new Token(StructuredPropertiesParser.Type.CLOSE_ARRAY, null, null, line);
                            case '=', ':' -> new Token(StructuredPropertiesParser.Type.EQUALS, previous.get().field, null, line);
        ...
        private static Token word(Token previous, String field, String sval, int line) {
            ...
            if (previousType.anyOf(Type.EQUALS, Type.FIELD_NAME)) {
                return new Token(StructuredPropertiesParser.Type.FIELD_VALUE, field, sval, line);
            } else if (previousType.anyOf(Type.INITIAL, Type.FIELD_VALUE, Type.CLOSE_ARRAY, Type.OPEN_OBJECT, Type.CLOSE_OBJECT)) {
                return new Token(StructuredPropertiesParser.Type.FIELD_NAME, sval, sval, line);
            } else if (previousType.anyOf(Type.OPEN_ARRAY, Type.ARRAY_VALUE)) {
                return new Token(StructuredPropertiesParser.Type.ARRAY_VALUE, field, sval, line);
            } else {
