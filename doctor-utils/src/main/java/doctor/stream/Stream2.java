...
import doctor.tuple.Tuple2Consumer;
import doctor.tuple.Tuple2Function;
import doctor.tuple.Tuple2Predicate;
...
public class Stream2<A, B> extends StreamExt<Tuple2<A, B>> {
    ...
    public <A2> Stream2<A2, B> mapLeft(Function<A, A2> mapper) {
        return map2((a, b) -> NTuple.of(mapper.apply(a), b));
    }

    public <B2> Stream2<A, B2> mapRight(Function<B, B2> mapper) {
        return map2((a, b) -> NTuple.of(a, mapper.apply(b)));
    }
    ...
    public <C> Stream3<A, B, C> affixRight(Tuple2Function<A, B, ? extends C> mapper) {
        return map3(t -> NTuple.of(t.first(), t.second(), mapper.apply(t)));
    }

    public <C> Stream3<A, B, C> flatAffixRight(Tuple2Function<A, B, Stream<? extends C>> mapper) {
        return new Stream3<>(flatMap(t -> mapper.apply(t).map(c -> NTuple.of(t.first(), t.second(), c))));
    }

    public <C> Stream3<C, A, B> affixLeft(Tuple2Function<A, B, ? extends C> mapper) {
        return map3(t -> NTuple.of(mapper.apply(t), t.first(), t.second()));
    }
}
